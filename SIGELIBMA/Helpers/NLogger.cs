    ...
    public sealed class NLogger : ITraceWriter
    {
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Exception != null && !string.IsNullOrWhiteSpace(record.Exception.GetBaseException().Message))
            {
                ...
                if (exceptionType == typeof(EmptyParametersException))
                {
                    var exception = record.Exception as EmptyParametersException;
                    if (exception != null)
                    {
                        message.Append("").Append("Error: " + exception.Message + Environment.NewLine);
                        message.Append("").Append("Inner Exception: " + exception.InnerException + Environment.NewLine);
                    }
                }
                else if (exceptionType == typeof(InvalidFormatException))
                {
                    var exception = record.Exception as InvalidFormatException;
                    if (exception != null)
                    {
                        message.Append("").Append("Error: " + exception.Message + Environment.NewLine);
                        message.Append("").Append("Inner Exception: " + exception.InnerException + Environment.NewLine);
                ...
                {
                    var exception = record.Exception as NotFoundException;
                    if (exception != null)
                    {
                        message.Append("").Append("Error: " + exception.Message + Environment.NewLine);
                        message.Append("").Append("Inner Exception: " + exception.InnerException + Environment.NewLine);
        ...
        public void LogFatal(string exception)
        {
            ...
            Logger[TraceLevel.Fatal](Convert.ToString(message) + Environment.NewLine);
        }
