...
var data = {};
data.autor = { codigo: 0, nombre : '', apellidos:"", estado:0 };
data.autores = [];
data.estados =[];
data.modalObject = { codigo: 0, nombre : '', apellidos:"", estado:0 };
    ...
    methods: {
        ...
        searchInTheList : function(searchText, currentPage) {
            if (_.isUndefined(searchText)) {
                ...
                this.filteredItems = _.filter(vm.items, function (v, k) {
                    if ((v.codigo!=null)&&(v.nombre!=null)&&(v.apellidos!=null)&&(v.estado!=null)) {
                        ...
                        | (!v.selected && v.apellidos.toLowerCase().indexOf(searchText.toLowerCase()) > -1)
        ...
        getInitData: function () {
                $.ajax({
                    url: urlRoot + 'MantAutor/ObtenerInitData',
                    type: 'get',
                    dataType: 'json',
        ...
        getAutores: function () {
            $.ajax({
                url: urlRoot + 'MantAutor/ObtenerTodos',
                type: 'get',
                dataType: 'json',
                success: function (result) {
                    if (result.EstadoOperacion) {
        ...
        agregar: function () {
            vm.displaySpinner(true,'Agregando Autor');
            $.ajax({
                url: urlRoot + 'MantAutor/Agregar',
                type: 'post',
                dataType: 'json',
                data: vm.autor,
                success: function (result) {
                    if (result.EstadoOperacion) {
                        vm.getAutores();     
                        vm.autor = { codigo: 0, nombre : '', apellidos:"", estado:0 };
                    } else {
        ...
        modificar: function () {
            ...
            vm.displaySpinner(true, 'Editando Autor');
            $.ajax({
                url: urlRoot + 'MantAutor/Modificar',
                type: 'post',
                dataType: 'json',
                data: vm.modalObject,
                success: function (result) {
        ...
        eliminar: function (autor) {
            vm.displaySpinner(true, 'Desabilitando Autor');
            autor.estado = 0;
            $.ajax({
                url: urlRoot + 'MantAutor/Desabilitar',
                type: 'post',
                dataType: 'json',
                data: autor,
                success: function (result) {
        ...
        openEditModal: function (autor) {
            vm.activateAlertModal('','',false);
            vm.modalObject = autor;
